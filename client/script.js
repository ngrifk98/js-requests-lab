...
// CODE HERE
function revertButtonStyle() {
    sayHelloButton.style.backgroundColor = '#EFEFEF';
    sayHelloButton.style.color = 'black';
}

sayHelloButton.addEventListener('mouseout', revertButtonStyle);
...
const sendQuery = () => {
    ...
    // TEST Scenario 1: Send NO queries on the URL
    ...
    // TEST Scenario 2: Sending more than 1 query on the URL
    axios.get(`http://localhost:3000/query-test?param1=value1&param2=value2`)
    .then((response) => {
        console.log(response.data);
    })
    .catch((error) => {
...
const createFood = (event) => {
    ...
    const newFood = foodInput.value.trim(); // Trim leading/trailing whitespace
  
    // Check if the input is empty or already exists in the list
    if (newFood === '') {
      return; // Skip adding empty food items
    }
  
    const foodList = document.getElementById('food-list');
  
    // Check if the new food item already exists in the list
    const existingFoods = Array.from(foodList.getElementsByTagName('li')).map((item) => item.textContent.toLowerCase());
    if (existingFoods.includes(newFood.toLowerCase())) {
      foodInput.value = ''; // Clear the input field
      return; // Skip adding duplicate food items
    }
